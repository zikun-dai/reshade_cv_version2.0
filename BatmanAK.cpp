...
std::string GameBatmanAK::gamename_verbose() const { return "GameBatmanAK"; } // hopefully continues to work with future patches via the mod lua

std::string GameBatmanAK::camera_dll_name() const { return ""; } // no dll name, it's available in the exe memory space
uint64_t GameBatmanAK::camera_dll_mem_start() const { return 0; }
GameCamDLLMatrixType GameBatmanAK::camera_dll_matrix_format() const { return GameCamDLLMatrix_allmemscanrequiredtofindscriptedcambuf; }

scriptedcam_checkbuf_funptr GameBatmanAK::get_scriptedcambuf_checkfun() const {
	return template_check_scriptedcambuf_hash<double, 13, 1>;
}
uint64_t GameBatmanAK::get_scriptedcambuf_sizebytes() const {
	return template_scriptedcambuf_sizebytes<double, 13, 1>();
}
bool GameBatmanAK::copy_scriptedcambuf_to_matrix(uint8_t* buf, uint64_t buflen, CamMatrixData& rcam, std::string& errstr) const {
	return template_copy_scriptedcambuf_extrinsic_cam2world_and_fov<double, 13, 1>(buf, buflen, rcam, false, errstr);
}

bool GameBatmanAK::can_interpret_depth_buffer() const {
	return true;
}
float GameBatmanAK::convert_to_physical_distance_depth_u64(uint64_t depthval) const {//把游戏深度缓冲区里的整数位（实际可能是浮点数），转换为物理世界的距离（单位是米）
	...
	uint32_t depth_as_u32 = static_cast<uint32_t>(depthval);//仅保留低32位
    float depth;
	std::memcpy(&depth, &depth_as_u32, sizeof(float));//把32位整数按位解释为浮点数
    depth = 1.0f - depth;
    const float n = 0.1f;//near
    const float f = 10000.0f;//far
    //把非线性深度反算回线性深度
    const float numerator_constant = (-f * n) / (n - f);
    const float denominator_constant = n / (n - f);
    return numerator_constant / (depth - denominator_constant);//这个算法是用near，far映射到[0,1]
}

uint64_t GameBatmanAK::get_scriptedcambuf_triggerbytes() const
{
...
void GameBatmanAK::process_camera_buffer_from_igcs(
