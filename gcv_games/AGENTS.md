...
1. 这个新的pipeline只适用于单帧截取吗，还是也适用于F9/F10的视频录制模式？
2. 这个新的pipeline必须要Enable DisplayDepth.fx才能正常跑通吗，因为我还需要视频数据，开着DisplayDepth.fx时整个画面会失真（左半屏是法线图、右半屏是深度图），我希望视频数据和深度数据能够同时正确采集。 -->
