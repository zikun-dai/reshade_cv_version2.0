...
// std::string GameBatmanAK::gamename_verbose() const { return "BatmanAK"; } // hopefully continues to work with future patches via the mod lua
...
// 	return true;
// }
// float GameBatmanAK::convert_to_physical_distance_depth_u64(uint64_t depthval) const {
//     // 1. 提取低32位深度数据（过滤模板缓冲）
//     uint32_t depth_as_u32 = static_cast<uint32_t>(depthval & 0xFFFFFFFF);
    
//     // 2. 字节序反转（若需）
//     depth_as_u32 = ((depth_as_u32 & 0x000000FF) << 24) |
//                    ((depth_as_u32 & 0x0000FF00) << 8) |
//                    ((depth_as_u32 & 0x00FF0000) >> 8) |
//                    ((depth_as_u32 & 0xFF000000) >> 24);
...
//     // 4. 虚幻3深度转换公式（非反向Z）
//     const float n = 0.1f;       // 近裁剪平面
//     float f = 10000.0f;         // 远裁剪平面（可通过实际场景校准）
    
//     // 代入公式计算线性深度
//     float linear_depth = (2.0f * f * n) / ( (f + n) - depth * (n - f) );
