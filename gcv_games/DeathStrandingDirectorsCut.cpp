...
#include "DeathStrandingDirectorsCut.h"
#include "gcv_utils/depth_utils.h"
#include "gcv_utils/scripted_cam_buf_templates.h"


std::string GameDeathStrandingDirectorsCut::gamename_verbose() const { return "DeathStrandingDirectorsCut"; } // hopefully continues to work with future patches via the mod lua

std::string GameDeathStrandingDirectorsCut::camera_dll_name() const { return ""; } // no dll name, it's available in the exe memory space
uint64_t GameDeathStrandingDirectorsCut::camera_dll_mem_start() const { return 0; }
GameCamDLLMatrixType GameDeathStrandingDirectorsCut::camera_dll_matrix_format() const { return GameCamDLLMatrix_allmemscanrequiredtofindscriptedcambuf; }

scriptedcam_checkbuf_funptr GameDeathStrandingDirectorsCut::get_scriptedcambuf_checkfun() const {
    return template_check_scriptedcambuf_hash<double, 13, 1>;
}
uint64_t GameDeathStrandingDirectorsCut::get_scriptedcambuf_sizebytes() const {
    return template_scriptedcambuf_sizebytes<double, 13, 1>();
}
bool GameDeathStrandingDirectorsCut::copy_scriptedcambuf_to_matrix(uint8_t* buf, uint64_t buflen, CamMatrixData& rcam, std::string& errstr) const {
    return template_copy_scriptedcambuf_extrinsic_cam2world_and_fov<double, 13, 1>(buf, buflen, rcam, true, errstr);
}

bool GameDeathStrandingDirectorsCut::can_interpret_depth_buffer() const {
    return true;
}
float GameDeathStrandingDirectorsCut::convert_to_physical_distance_depth_u64(uint64_t depthval) const {
...
uint64_t GameDeathStrandingDirectorsCut::get_scriptedcambuf_triggerbytes() const
{
...
void GameDeathStrandingDirectorsCut::process_camera_buffer_from_igcs(
    ...
    float fov)
{
    ...
    const float M_to_GL[3][3] = { { 0, 1, 0 }, { 0, 0, -1 }, { 1, 0, 0 } };
    const float M_T[3][3] = { { 0, 0, 1 }, { 1, 0, 0 }, { 0, -1, 0 } };

    float temp_matrix[3][3] = { 0.0f };
    for (int i = 0; i < 3; ++i) for (int j = 0; j < 3; ++j) for (int k = 0; k < 3; ++k)
        temp_matrix[i][j] += R_matrix[i][k] * M_T[k][j];

    float R_gl_final[3][3] = { 0.0f };
    for (int i = 0; i < 3; ++i) for (int j = 0; j < 3; ++j) for (int k = 0; k < 3; ++k)
        R_gl_final[i][j] += M_to_GL[i][k] * temp_matrix[k][j];

    const float pose_scale = 1.0f;
    float t_cv_final[3] = {
        camera_ue_pos[0] * pose_scale,
        camera_ue_pos[2] * pose_scale,
        -camera_ue_pos[1] * pose_scale
    };

    camera_data_buffer[2] = R_gl_final[0][0];
    camera_data_buffer[3] = R_gl_final[0][1];
    camera_data_buffer[4] = R_gl_final[0][2];
    camera_data_buffer[5] = t_cv_final[0];

    camera_data_buffer[6] = R_gl_final[1][0];
    camera_data_buffer[7] = R_gl_final[1][1];
    camera_data_buffer[8] = R_gl_final[1][2];
    camera_data_buffer[9] = t_cv_final[1];

    camera_data_buffer[10] = R_gl_final[2][0];
    camera_data_buffer[11] = R_gl_final[2][1];
    camera_data_buffer[12] = R_gl_final[2][2];
