...
std::string GameGodOfWar::gamename_verbose() const { return "GameGodOfWar"; } // hopefully continues to work with future patches via the mod lua

std::string GameGodOfWar::camera_dll_name() const { return ""; } // no dll name, it's available in the exe memory space
uint64_t GameGodOfWar::camera_dll_mem_start() const { return 0; }
GameCamDLLMatrixType GameGodOfWar::camera_dll_matrix_format() const { return GameCamDLLMatrix_allmemscanrequiredtofindscriptedcambuf; }

scriptedcam_checkbuf_funptr GameGodOfWar::get_scriptedcambuf_checkfun() const {
    return template_check_scriptedcambuf_hash<double, 13, 1>;
}
uint64_t GameGodOfWar::get_scriptedcambuf_sizebytes() const {
    return template_scriptedcambuf_sizebytes<double, 13, 1>();
}
bool GameGodOfWar::copy_scriptedcambuf_to_matrix(uint8_t* buf, uint64_t buflen, CamMatrixData& rcam, std::string& errstr) const {
    return template_copy_scriptedcambuf_extrinsic_cam2world_and_fov<double, 13, 1>(buf, buflen, rcam, false, errstr);
}

bool GameGodOfWar::can_interpret_depth_buffer() const {
    return true;
}
float GameGodOfWar::convert_to_physical_distance_depth_u64(uint64_t depthval) const {
...
uint64_t GameGodOfWar::get_scriptedcambuf_triggerbytes() const
{
...
void GameGodOfWar::process_camera_buffer_from_igcs(
    ...
    float fov)
{
    ...
    const float cr = cos(pitch), sr = sin(pitch);
    const float cp = cos(yaw), sp = sin(yaw);
    const float cz = cos(roll), sz = sin(roll);
    ...
    //};
    const float t_cv[3] = {
        camera_ue_pos[0] * scale, // t_cv[0] = y
        camera_ue_pos[1] * scale, // t_cv[1] = -z
        camera_ue_pos[2] * scale  // t_cv[2] = x
    };
    ...
    // 第一行
    camera_data_buffer[2] = R_ue[0][0];
    camera_data_buffer[3] = R_ue[0][1];
    camera_data_buffer[4] = R_ue[0][2];
    camera_data_buffer[5] = t_cv[0];
    // 第二行
    camera_data_buffer[6] = R_ue[1][0];
    camera_data_buffer[7] = R_ue[1][1];
    camera_data_buffer[8] = R_ue[1][2];
    camera_data_buffer[9] = t_cv[1];
    // 第三行
    camera_data_buffer[10] = R_ue[2][0];
    camera_data_buffer[11] = R_ue[2][1];
    camera_data_buffer[12] = R_ue[2][2];
    camera_data_buffer[13] = t_cv[2];
    // FOV
