...
std::string GameGodofWar5_CE::gamename_verbose() const { return "GodofWar5_CE"; }

std::string GameGodofWar5_CE::camera_dll_name() const { return ""; } // no dll name, it's available in the exe memory space
uint64_t GameGodofWar5_CE::camera_dll_mem_start() const { return 0; }
GameCamDLLMatrixType GameGodofWar5_CE::camera_dll_matrix_format() const { return GameCamDLLMatrix_allmemscanrequiredtofindscriptedcambuf; }

scriptedcam_checkbuf_funptr GameGodofWar5_CE::get_scriptedcambuf_checkfun() const {
	return template_check_scriptedcambuf_hash<double, 13, 1>;
}
uint64_t GameGodofWar5_CE::get_scriptedcambuf_sizebytes() const {
	return template_scriptedcambuf_sizebytes<double, 13, 1>();
}
bool GameGodofWar5_CE::copy_scriptedcambuf_to_matrix(uint8_t* buf, uint64_t buflen, CamMatrixData& rcam, std::string& errstr) const {
	// 先使用 IGCS scriptedcambuf 获取 FOV（和初始矩阵）
	if (!template_copy_scriptedcambuf_extrinsic_cam2world_and_fov<double, 13, 1>(buf, buflen, rcam, false, errstr)) {
		return false;
	}
	// 然后用内存中的矩阵覆盖 extrinsic_cam2world
	return overwrite_extrinsic_from_memory(rcam, errstr);
}

bool GameGodofWar5_CE::can_interpret_depth_buffer() const {
	return true;
}
float GameGodofWar5_CE::convert_to_physical_distance_depth_u64(uint64_t depthval) const {
...
uint64_t GameGodofWar5_CE::get_scriptedcambuf_triggerbytes() const
{
...
void GameGodofWar5_CE::process_camera_buffer_from_igcs(
...
bool GameGodofWar5_CE::overwrite_extrinsic_from_memory(CamMatrixData& rcam, std::string& errstr) const
{
	const HMODULE exe_module = GetModuleHandleW(nullptr);
	if (exe_module == nullptr) {
		errstr += "[GodofWar5_CE] failed to get GoWR.exe module handle for c2w matrix";
		return false;
	}
	...
	// 步骤 1: 从内存读取 4*3 矩阵（12个float）
	float cambuf[12] = {};
	const uint64_t cam_matrix_offset = 0x376E54Cull;
	if (!tryreadmemory(gamename_verbose() + std::string("_4x3cam"), errstr, mygame_handle_exe,
		(LPCVOID)(exe_base + cam_matrix_offset), reinterpret_cast<LPVOID>(cambuf),
		sizeof(cambuf), &nbytesread)) {
		return false;
	}

	// 步骤 2: 转置 4*3 -> 3*4
	// 原始矩阵是 4行3列，按行优先存储在 cambuf 中
	float transposed[3][4];
	for (int i = 0; i < 3; ++i) {
		for (int j = 0; j < 4; ++j) {
			transposed[i][j] = cambuf[j * 3 + i];
		}
	}

	// // 步骤 3: 对rot第二列取负（索引为1）
	// for (int i = 0; i < 3; ++i) {
	// 	transposed[i][1] = -transposed[i][1];
	// }

	// // 步骤 4: 对rot第三行取负（索引为2）
	// for (int j = 0; j < 3; ++j) {
	// 	transposed[2][j] = -transposed[2][j];
	// }

	// // 步骤 5: 第四列重排 [x,y,z] -> [y,-z,x]
	// float x = transposed[0][3];
	// float y = transposed[1][3];
	// float z = transposed[2][3];
	// transposed[0][3] = y;
	// transposed[1][3] = -z;
	// transposed[2][3] = x;

	// 步骤 6: 填入 rcam.extrinsic_cam2world（3*4 矩阵）
	for (int i = 0; i < 3; ++i) {
		for (int j = 0; j < 4; ++j) {
			rcam.extrinsic_cam2world(i, j) = transposed[i][j];
		}
	}
